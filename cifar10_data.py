...
class Cifar10():
    ...
    def _loadDatasetFromFiles(self):
        ...
        self.train_images = np.concatenate(train_data_list, 0).reshape([50000,3,32,32]).transpose([0,2,3,1])
        self.train_labels = np.concatenate(train_labels_list, 0)
        
        test_dict = _unpickle(os.path.join(file_dir, 'test_batch'))
        self.test_images = np.array(test_dict[b'data']).reshape([10000,3,32,32]).transpose([0,2,3,1])
